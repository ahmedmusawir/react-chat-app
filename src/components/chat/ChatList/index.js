...
import NewChatFormMoose from './NewChatFormMoose';
...
const ChatList = (props) => {
  ...
  const { conn, chats, setChats, setActiveChat } =
    useContext(ChatEngineContext);

  // console.log('CHAT LIST PROPS', props);
  ...
  function renderChats(chats) {
    return chats.map((chat, index) => {
      if (!chat) {
        ...
        return (
          <div key={`chat_${index}`}>{props.renderChatCard(chat, index)}</div>
        );
      } else {
        return (
          <div
    ...
    <div style={styles.chatListContainer} className="ce-chat-list">
      <div style={styles.chatsContainer} className="ce-chats-container">
        ...
        {hasMoreChats && chatList.length > 0 && (
          <div>
            <ChatLoader onVisible={() => !loadChats && setLoadChats(true)} />
